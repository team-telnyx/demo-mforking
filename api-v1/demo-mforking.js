...
function call_control_fork_start(f_call_control_id, f_fork_dest, f_rx, f_tx) {
...
function call_control_fork_stop(f_call_control_id) {

    var cc_action = 'fork_stop'
...
rest.post('/' + g_appName + '/mforking', function (req, res) {
    ...
    if (l_hook_event_type == 'call_initiated') {
        ...
        console.log("[%s] DEBUG - RECEIVED DTMF [%s]", get_timestamp(), l_dtmf_number);
        ...
        if (!l_client_state_64) { // IVR Lobby
            ...
            if (l_client_state_s == "stage-dial" && l_dtmf_number) {
                ...
                // Transfer Call
                call_control_transfer(l_call_control_id, g_pstn_destination, req.body.payload.from);

                // Start Forking

                // call_control_fork_start(f_call_control_id, f_fork_dest, f_rx, f_tx)
                call_control_fork_start(l_call_control_id, g_udp_dest, null, null);
